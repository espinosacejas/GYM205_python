...
def personal_account(request):
    ...
    try:
        dirname = f'accounts_cabinet/files_students/{result_login}/'
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            shutil.move(f'media/students_files/{filename_to_find}',
                        f'accounts_cabinet/files_students/{result_login}/{filename_to_find}')
            # переименовываем файл на всякий, даем в начале его id пользователя
            os.rename(f'accounts_cabinet/files_students/{result_login}/{filename_to_find}',
                      f'accounts_cabinet/files_students/{result_login}/{user_id}_{filename_to_find}')
